...
public class ProbeDAL
{
    ...
    {
        string sql = @"select ID,MailAddress,ProbeName,GasKind,Unit,FirstAlarmValue,SecondAlarmValue,MachineName,MachineID,PosDir,FactoryID,FactoryName,MachineType,Pos2D,TagName from Probe where 1=1 ";
    ...
    {
        string sql = @"select ID,MailAddress,ProbeName,GasKind,Unit,FirstAlarmValue,SecondAlarmValue,MachineName,MachineID,PosDir,FactoryID,FactoryName,MachineType,Pos2D,TagName from Probe where ID = @ID";
    ...
    public static bool EditProbeByID(int id, string mailAddress, string probeName, int machineID, string gasKind, string unit, string firstAlarmValue, string secondAlarmValue, string machineName,string tagName)
    {
        //没必要更新历史数据
        string sql = @"update Probe set MailAddress=@MailAddress,ProbeName=@ProbeName,GasKind=@GasKind,Unit=@Unit,FirstAlarmValue=@FirstAlarmValue,SecondAlarmValue=@SecondAlarmValue,MachineID=@MachineID,MachineName=@MachineName,TagName=@TagName where ID=@ID;
    ...
    public static int InsertProbe(string mailAddress, string probeName, string gasKind, string unit, string firstAlarmValue, string secondAlarmValue, string posdir, int machineID, string machineName, int factoryID, string factoryName, int machineType, string tagName)
    {
        string sql = @"insert into Probe (MailAddress,ProbeName,GasKind,Unit,FirstAlarmValue,SecondAlarmValue,MachineName,MachineID,PosDir,FactoryID,FactoryName,MachineType,TagName)values(@MailAddress,@ProbeName,@GasKind,@Unit,@FirstAlarmValue,@SecondAlarmValue,@MachineName,@MachineID,@PosDir,@FactoryID,@FactoryName,@MachineType,@TagName) SELECT @@IDENTITY AS ID;";
