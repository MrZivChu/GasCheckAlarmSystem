...
{
    class DZ40OldProtocol : ProtocolBase
    {
        ...
        public override bool HandleReceiveData(byte[] buffer, int bytesToReadCount)
        {
            ...
            for (int j = 0; j < machineSerialPortInfo_.list.Count; j++)
            {
                ...
                int index = (Convert.ToInt32(probeSerialPortInfo.ProbeAddress) - 1) * 4 + 2 + 3;
        ...
        public override void HandleSendData(MachineSerialPortInfo machineSerialPortInfo, out string sendContent)
        {
            ...
            int firstDec = (Convert.ToInt32(machineSerialPortInfo_.FirstProbeDecAddress) - 1) * 2;
            int endDec = (Convert.ToInt32(machineSerialPortInfo_.EndProbeDecAddress) - Convert.ToInt32(machineSerialPortInfo_.FirstProbeDecAddress) + 1) * 2;
            string firstHex = firstDec.ToString("X4");
            string endHex = endDec.ToString("X4");

            StringBuilder sb = new StringBuilder();
            sb.Append(Convert.ToInt32(machineSerialPortInfo_.MachineAddress).ToString("X2")).Append(machineSerialPortInfo.command).Append(firstHex).Append(endHex);
