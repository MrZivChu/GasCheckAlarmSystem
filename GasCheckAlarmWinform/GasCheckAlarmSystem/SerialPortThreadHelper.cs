...
public class SerialPortThreadHelper
{
    ...
    public void HandleMachineData()
    {
        ...
        catch (Exception ex)
        {
            ...
            File.WriteAllText("C:\\HandleMachineData.txt", ex.Message);
        }
    }

    void StartThread()
    {
        for (int i = 0; i < machineSerialPortInfoBaseList_.Count; i++)
        {
            MachineSerialPortInfo item = machineSerialPortInfoBaseList_[i];
            Thread thread = new Thread(StartSerialPort);
            thread.Start(item);
        }
    }

    void StartSerialPort(object obj)
    {
        MachineSerialPortInfo data = (MachineSerialPortInfo)obj;
        ProtocolBase protocol;
        SerialPortConnect(protocol, portName_, 9600, 8, Parity.None, StopBits.One);
    }

    void SerialPortConnect(ProtocolBase protocol, string portName, int baudRate, int dataBits, Parity parity = Parity.None, StopBits stopBits = StopBits.One)
    {
        try
        {
